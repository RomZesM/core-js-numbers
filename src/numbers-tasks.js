 ...
 */
function getDistanceBetweenPoints(a1, b1, a2, b2) {
 ...
 *   5*x = 0         => 0
 */
function getLinearEquationRoot(a1, b1) {
  return (b1 * -1) / a1;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let rad = Math.atan2(y2, x2) - Math.atan2(y1, x1);
  if (rad < 0) {
    rad += 2 * Math.PI;
  }
  return rad;
...
function parseNumberFromString(val) {
  ...
  } else ret = Number.parseInt(val, 10);
  return ret;
}
